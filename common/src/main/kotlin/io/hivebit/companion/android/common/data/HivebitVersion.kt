...
data class HivebitVersion(val year: Int, val month: Int, val release: Int) {

    companion object {
        private val VERSION_PATTERN = Pattern.compile("([0-9]{4})\\.([0-9]{1,2})\\.([0-9]{1,2}).*")

        fun fromString(versionString: String): HivebitVersion? {
            ...
            return if (matches.find() && matches.matches()) {
                ...
                HivebitVersion(coreYear, coreMonth, coreRelease)
